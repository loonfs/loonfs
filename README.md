...
The first public installer target is macOS arm64. It installs `loon` and `loond` into
`~/.loonfs/bin` and prepares:

- `~/.config/loonfs/loon/` for CLI-managed profile state
- `~/.config/loonfs/loond/` for user-authored server configs
...
- macOS: `~/.config/loonfs/loon/config.toml`
...
Sanitized example configs live in both:

- the installed macOS example root at `~/.config/loonfs/loond/examples/`
- the repository source tree under [`configs/`](configs/)
...
Remote profiles remain available for already-running servers:
...
Smoke acceptance from the repository:
