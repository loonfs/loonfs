...
1. On the server machine, copy
   ...
   and fill in the real R2 bucket, account, endpoint, credentials, and any desired `key_prefix`.
2. On both client machines, copy
   ...
   and point `server_url` at the server host and `auth_token` at the shared bearer token.
...
```bash
cargo run -p loon-server --bin loond -- --config ./configs/loond.cloudflare-r2.example.toml
```
...
```bash
cargo run -p loon-cli -- --config ./configs/loon-client.r2.example.toml namespace create demo

cargo run -p loon-cli -- --config ./configs/loon-client.r2.example.toml \
...
```bash
cargo run -p loon-cli -- --config ./configs/loon-client.r2.example.toml namespace list
