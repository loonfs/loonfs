...
A GC response includes `next_reclamation_at_ms` when a deleted generation is inside its retirement grace or an upload session has a future cleanup time. It is the earliest of those future times examined by the pass. A generation that pins still hold after its grace carries no time here. Upload cleanup times include lease plus grace, abort grace, and completed-content grace. Candidates that age out by provider timestamps carry no time here. Absence does not mean that nothing remains to collect.
