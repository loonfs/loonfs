...
The embedded runtime may overlap a small buffered content read with current-path validation. It resolves a candidate from a cached view without consuming local publication evidence, then performs the ordinary freshness and path checks. Speculative bytes are usable only when the current path resolves to the same namespace-bound content store and complete content reference; the returned entry comes from the current view. Current metadata errors take precedence. An obsolete content result is discarded before fetching a changed reference once.
