...
An import resolves the source reference through its owner's current view. It verifies resident inline bytes or streams the content object, then stages the bytes under a fresh destination-owned identity.
