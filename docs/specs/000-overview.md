...
| Goal | Meaning |
| --- | --- |
| **Simple** | The durable model should fit in a small number of concepts: namespaces, inodes, revisions, content refs, logical commits, WAL segments, and manifests. |
...
| Recovery | Readers reconstruct state from a verified manifest plus the visible WAL segment chain after that manifest. |
