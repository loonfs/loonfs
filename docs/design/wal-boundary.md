...
- **Garbage collection.** Object enumeration, age checks, and deletion remain in `gc/`. For a live namespace, an object remains required if its number is above the folded position. An object at or below it is eligible for the remaining collection checks.
