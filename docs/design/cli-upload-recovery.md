...
Before it sends any content, the CLI records the chosen commit ID, actor, message, overwrite preconditions, source length, and full-precision source modification time.
...
Source validation compares file metadata, not a full-file hash. A source change that preserves both the recorded length and modification time is outside that check. A prepared request still refers to the content that was uploaded originally, rather than to the bytes the local path holds at retry time.
...
Once a request is prepared, recovery does not reopen the upload session. The CLI resends the saved request directly. If a multipart session already completed on the server before the request was prepared, the CLI reads the completed session back and commits without uploading again. This matters when the server committed the request but the response was lost: the durable commit receipt can establish the original outcome even if the upload session has since been removed.
...
An exclusive sidecar-file lock is held for each file PUT; a recursive upload takes and releases it per file. A competing command fails immediately rather than operating on the same upload session or recovery record.
