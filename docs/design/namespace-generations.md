...
- **Commit retries.** Commit receipts are metadata rows and do not cross the boundary. A retry of a commit id from an earlier generation executes as a new commit in the current one. Any name-addressed system has this property.
