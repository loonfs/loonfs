...
#[test]
fn local_only_directory_fixture_persists_remote_create_plan() {
    ...
    db.planner_transaction("seed-local-only-directory-fixture", |tx| {
        tx.upsert_local_only_file(&initial.local_only_state)?;
        Ok(())
    })
    .expect("seed local-only directory fixture");

    assert_eq!(actions.len(), 1, "fixture should contain one planner tick");
    let action = &actions[0].planner_tick_local_only;
    let planned = plan_local_only_inode(&mut db, &action.client_file_id, action.now_ms)
        .expect("local-only directory planner should succeed");
