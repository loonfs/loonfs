...
fn local_only_directory_binding_fixture_migrates_temp_identity_into_inode_state() {
    let scenario = load_fixture("client/local_only_directory_binds_to_remote_inode.yaml");
    ...
    db.planner_transaction("seed-local-only-directory-bind-fixture", |tx| {
    ...
    db.bind_local_only_inode_to_remote(&bind.client_file_id, &bind.remote_state)
        .expect("bind local-only directory identity");
