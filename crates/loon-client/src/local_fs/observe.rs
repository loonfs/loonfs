use super::paths::{relative_path_is_under_prefix, NamespacePathIndex};
...
pub fn observe_local_path(
    ...
    require_existing_file(state_db_path, "client state db")?;
    ...
    let mirror_root = fs::canonicalize(mirror_root)?;
    ...
    let parent_relative_path = Path::new(&relative_path)
        .parent()
        .map(normalize_relative_path)
        .unwrap_or_default();
    ...
    let mut db = SqliteStateDb::open(state_db_path)?;
...
pub fn observe_move_path(
    ...
    let parent_relative_path = Path::new(&to_relative_path)
        .parent()
        .map(normalize_relative_path)
        .unwrap_or_default();
...
fn require_existing_file(path: &Path, label: &str) -> Result<(), IoError> {
    let metadata = fs::metadata(path).map_err(|err| {
        IoError::new(
            err.kind(),
            format!("required {label} missing at {}", path.display()),
        )
    })?;
    if !metadata.is_file() {
        return Err(IoError::new(
            IoErrorKind::InvalidInput,
            format!("required {label} is not a file: {}", path.display()),
        ));
    }
    Ok(())
}
