...
/// Provider operation deadline for small control objects, in milliseconds
/// (`loonfs-objectstore` consumes it across every retry of one logical
/// operation). Payload-bearing writes extend it by payload size; the floor
/// derivation below stays valid because every object it times — WAL
/// segments inside the publish budget, checkpoint records, the root
/// compare-and-swap — is a small control object whose deadline reduces to
/// this base, and publications self-enforce their budgets by wall clock
/// regardless of how long any single provider operation was allowed to run.
pub const PROVIDER_OP_DEADLINE_MS: u64 = PROVIDER_OP_DEADLINE.as_millis() as u64;

/// One provider HTTP attempt's base request timeout, in milliseconds
/// (payload-bearing request phases extend it by payload size). A
/// small-object operation's total wall time is bounded by
