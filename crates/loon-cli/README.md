...
than being re-specified in `loon-cli`. In particular, any subtree move pairing semantics should
come from `loon-ops` unchanged: unique digest-equal file pairs may infer a move, while directory
moves still require explicit `observe-move`.
