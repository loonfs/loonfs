...
use loon_testkit::invariants::{
    ...
    BackgroundRepairInvariantInputs, BackgroundStaleWriterInvariantInputs,
    BackgroundWorkInvariantReport, CheckpointHeadPublishInvariantInputs,
    CheckpointProgressAuthorizer, ProgressInvariantSnapshot, ProgressPublishInvariantInputs,
    ProgressPublishOutcomeKind, QueueRepairInvariantInputs, QueueRepairOutcomeKind,
    QueueShardObjectInvariantInputs, SimInvariantReport,
};
...
pub enum BackgroundSimRunStatus {
    Passed,
...
pub struct BackgroundExplorationCaseLabel {
...
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct BackgroundSimRunOptions {
    pub exploration_case: Option<BackgroundExplorationCaseLabel>,
}

#[derive(Debug, Clone)]
pub struct BackgroundFixtureRunReport {
    pub rendered_trace: String,
    pub status: BackgroundSimRunStatus,
}

#[allow(dead_code)]
pub fn run_background_fixture_report(relative_path: &str) -> BackgroundFixtureRunReport {
    let scenario = load_fixture(relative_path);
    run_background_scenario_report(&scenario, BackgroundSimRunOptions::default())
}

pub fn run_background_scenario_report(
    scenario: &Scenario,
    options: BackgroundSimRunOptions,
        ...
        for (index, action) in actions.iter().enumerate() {
            match action.kind() {
                ...
                BackgroundSimActionRef::CheckpointBuild => {
                    ...
                    let core_checkpoint = prepare_checkpoint(
                        &core_head,
                        &initial.metadata_state,
                        TEST_WRITER_VERSION,
                    )
                    ...
                    if checkpoint_publish_history.len() >= 2 {
                        ...
                        let sim_report = evaluate_background_checkpoint_publish_invariants(
                            BackgroundCheckpointPublishInvariantInputs {
                                first_publish_blocked: first
                                    .outcome
                                    .is_blocked_for_required_progress(),
                                ...
                                retention_floor_after_success: second
                                    .after_head
                                    .retention_floor_seq,
                            },
                        );
    ...
    let status = match execution {
        Ok(()) => BackgroundSimRunStatus::Passed,
        Err(payload) => {
            let headline = panic_payload_to_headline(payload);
            if let Some(reason) = background_not_executable_reason(&headline) {
                BackgroundSimRunStatus::NotExecutable { reason }
            } else {
                BackgroundSimRunStatus::Failed {
...
fn background_not_executable_reason(headline: &str) -> Option<String> {
    if headline.contains("writer_attempt_commit requires writer_read_current_head first") {
        return Some("writer_attempt_commit requires a prior writer_read_current_head".to_owned());
    }
    if headline.contains("checkpoint_publish requires a prior checkpoint_build action") {
        return Some("checkpoint_publish requires a prior checkpoint_build".to_owned());
    }
    None
}
...
fn panic_payload_to_headline(payload: Box<dyn std::any::Any + Send>) -> String {
    match payload.downcast::<String>() {
        ...
        Err(payload) => match payload.downcast::<&'static str>() {
            Ok(message) => message.lines().next().unwrap_or("").to_owned(),
            Err(_) => "background sim panicked".to_owned(),
        },
    }
}
