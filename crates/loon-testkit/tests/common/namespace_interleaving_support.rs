...
pub enum NamespaceSimRunStatus {
    Passed,
...
pub struct NamespaceExplorationCaseLabel {
...
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct NamespaceSimRunOptions {
    pub exploration_case: Option<NamespaceExplorationCaseLabel>,
}

#[derive(Debug, Clone)]
pub struct NamespaceFixtureRunReport {
    pub rendered_trace: String,
    #[allow(dead_code)]
    pub status: NamespaceSimRunStatus,
}

#[allow(dead_code)]
pub fn run_namespace_fixture_report(relative_path: &str) -> NamespaceFixtureRunReport {
    let scenario = load_fixture(relative_path);
    run_namespace_scenario_report(&scenario, NamespaceSimRunOptions::default())
}

pub fn run_namespace_scenario_report(
    scenario: &Scenario,
    options: NamespaceSimRunOptions,
    ...
    #[derive(Debug)]
    struct NotExecutableStep {
        reason: String,
    }

    let execution = catch_unwind(AssertUnwindSafe(|| -> Result<(), NotExecutableStep> {
        ...
        for (index, action) in actions.iter().enumerate() {
            match action.kind() {
                ...
                NamespaceSimActionRef::DeliverNextRequest => {
                    let Some(delivery) = runtime.take_next_matching_delivery(|delivery| {
                        delivery.kind == "client_request" && delivery.recipient.as_str() == "server"
                    }) else {
                        return Err(NotExecutableStep {
                            reason:
                                "deliver_next_request requires a queued client_request delivery"
                                    .to_owned(),
                        });
                    };
                    push_latest_runtime_trace_line(&mut trace, &runtime);
                    let Some(queued) = request_deliveries.remove(&delivery.delivery_id) else {
                        return Err(NotExecutableStep {
                            reason: "deliver_next_request requires a queued request payload"
                                .to_owned(),
                        });
                    };
                ...
                NamespaceSimActionRef::ServerHandleNextRequest(action) => {
                    let Some(queued) = server_inbox.pop_front() else {
                        return Err(NotExecutableStep {
                            reason:
                                "server_handle_next_request requires a queued server inbox request"
                                    .to_owned(),
                        });
                    };
                    let request = queued.into_request().expect("server inbox request payload");
                ...
                NamespaceSimActionRef::DeliverNextResponse => {
                    let Some(delivery) = runtime.take_next_matching_delivery(|delivery| {
                        ...
                        return Err(NotExecutableStep {
                            reason:
                                "deliver_next_response requires a queued client_response delivery"
                                    .to_owned(),
                        });
                    };
                    push_latest_runtime_trace_line(&mut trace, &runtime);
                    let Some(response) = response_deliveries.remove(&delivery.delivery_id) else {
                        return Err(NotExecutableStep {
                            reason: "deliver_next_response requires a queued response payload"
                                .to_owned(),
                        });
                    };
                    let mut db = SqliteStateDb::open(&db_path).expect("open client DB");
                ...
                NamespaceSimActionRef::DeliverRemoteObservation(action) => {
                    let Some(delivery) = runtime.take_next_matching_delivery(|delivery| {
                        ...
                        return Err(NotExecutableStep {
                        reason:
                            "deliver_remote_observation requires a queued remote_observation delivery"
                                .to_owned(),
                    });
                    };
                    push_latest_runtime_trace_line(&mut trace, &runtime);
                    let Some(seeded) = observation_deliveries.remove(&delivery.delivery_id) else {
                        return Err(NotExecutableStep {
                            reason:
                                "deliver_remote_observation requires a queued observation payload"
                                    .to_owned(),
                        });
                    };
                ...
                NamespaceSimActionRef::WriterAttemptCommit(action) => {
                    let Some(view) = cached_writer_view.as_ref() else {
                        return Err(NotExecutableStep {
                        reason:
                            "writer_attempt_commit requires a prior writer_read_current_head action"
                                .to_owned(),
                    });
                    };
                    if view.planned_head_seq == core_head.seq
                        && view.writer_fence_token == core_head.active_fence_token
                        && view.writer_id == lease.holder_id
                        && lease.fence_token == core_head.active_fence_token
                    {
                        return Err(NotExecutableStep {
                        reason:
                            "writer_attempt_commit requires a stale cached writer view after handover"
                                .to_owned(),
                    });
                    }
                    let now_ms = runtime
                    ...
                    .unwrap_or_else(|err| {
                        CommitAttemptOutcome::Rejected(normalize_core_error(err))
                    });
                    ...
                    stale_publish_rejected =
                        matches!(core_outcome, CommitAttemptOutcome::Rejected(_));
                ...
                NamespaceSimActionRef::CheckpointPublish(action) => {
                    ...
                    let Some(prepared_core) = prepared_checkpoint.as_ref() else {
                        return Err(NotExecutableStep {
                            reason: "checkpoint_publish requires a prior checkpoint_build action"
                                .to_owned(),
                        });
                    };
                    let Some(prepared_model) = prepared_model_checkpoint.as_ref() else {
                        return Err(NotExecutableStep {
                            reason: "checkpoint_publish requires a prior checkpoint_build action"
                                .to_owned(),
                        });
                    ...
                    if let NormalizedCheckpointPublishOutcome::Published { .. } = core_outcome {
                        ...
                        let checkpoint_report = evaluate_checkpoint_head_publish_invariants(
                            CheckpointHeadPublishInvariantInputs {
                                ...
                                checkpoint_namespace: &loaded_checkpoint
                                    .manifest
                                    .payload
                                    .namespace_id,
                ...
                NamespaceSimActionRef::RepairLostEnqueueToQueueShard(action) => {
                    ...
                    let queue_report =
                        evaluate_queue_repair_invariants(QueueRepairInvariantInputs {
                            ...
                            has_namespace_scoped_job_after: loaded_queue
                                .envelope
                                .state
                                .jobs
                                .iter()
                            ...
                            follow_up_through_seq_after: loaded_queue
                                ...
                                .and_then(|job| {
                                    job.follow_up.as_ref().map(|payload| payload.through_seq)
                                }),
                        });
                    assert_background_report_passes(
                        scenario,
        ...
        if expect.invariants.iter().any(|name| {
            ...
            let repair_tracking = last_repair_tracking.expect(
                "repair invariants require a prior repair_lost_enqueue_to_queue_shard action",
            );
            if expect
                .invariants
                .iter()
    ...
    let status = match execution {
        Ok(Ok(())) => NamespaceSimRunStatus::Passed,
        Ok(Err(not_executable)) => NamespaceSimRunStatus::NotExecutable {
            reason: not_executable.reason,
        },
        Err(panic) => NamespaceSimRunStatus::Failed {
            failure_headline: panic_payload_to_headline(panic.as_ref()),
        },
    };
...
fn panic_payload_to_headline(panic: &(dyn std::any::Any + Send)) -> String {
    ...
    "namespace sim execution panicked".to_owned()
}
