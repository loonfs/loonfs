use loon_client::state_db::{
    ...
    RemoteFileStateRow, SyncAnchorRow,
};
...
impl NamespacePathIndex {
    pub(crate) fn build(summary: &ClientNamespaceStateSummary) -> Self {
        ...
        let mut local_only_paths_by_client_file_id = BTreeMap::new();
        ...
        for row in &summary.local_only_state {
            if row.inode_kind != loon_types::InodeKind::File {
                continue;
            }
            let Some(parent_inode_id) = row.parent_inode_id else {
                continue;
            };
            let Some(parent_path) = local_paths_by_inode.get(&parent_inode_id) else {
                continue;
            };
            let path = join_relative_path(parent_path, &row.display_name);
            local_only_paths_by_client_file_id
                .insert(row.client_file_id.as_str().to_owned(), path.clone());
